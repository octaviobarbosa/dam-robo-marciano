...
open class Marciano() {
    ...
    open fun responda(comando: String): String {
        return when {
            comando.isNullOrBlank() -> vazio
            comando.contains("eu").or(pergunta.contains("Eu").or(pergunta.contains("EU"))) -> eu
            comando.contains("?").and(comando.split(" ").any { it.uppercase().equals(it) }) -> pergunta_grito
            comando.split(" ").any { it.uppercase().equals(it) } -> grito
